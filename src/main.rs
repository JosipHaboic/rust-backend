...
async fn main() -> std::io::Result<()> {
    ...
    let services = ServiceRegister::from([
        ("app".to_string(), ServiceInfo::new("app", &format!("{}:{}", address, port_0))),
        ("math-service".to_string(), ServiceInfo::new("math-service", &format!("{}:{}", address, port_1))),
    ]);
    ...
    let app_server = HttpServer::new(move || {
        App::new()
            .data(state::Application {
                ...
                services: [(
                    "math".to_owned(),
                    jsonrpc_v2_client::ServiceAddress {
    ...
    .bind(&services.get("app").unwrap().address)
    .unwrap()
    .run();

    let math_service_server = HttpServer::new(move || App::new().configure(math_service::config))
        .bind(&services.get("math-service").unwrap().address)
        .unwrap()
        .run();
